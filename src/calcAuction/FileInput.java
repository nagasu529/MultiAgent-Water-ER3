...
public class FileInput extends DatabaseConn {
	...
	public void farmFactorValues(List<String> inputList, ArrayList<cropType> outputInArrayList) {
		...
		while (listSize != 1) {
			...
			outputInArrayList.add(new FileInput.cropType(order, cropName, cropStage, droughtSensitivity, plotSize, yieldAmount,
	...
	public String calcCropEU(ArrayList<FileInput.cropType> inputArrayList, int decisionMethod) {
	...
	public String calcWaterReduction(double reductionPct, ArrayList<FileInput.cropType> inputArray, String agentName, double waterConsertCost) {
		//Preparing.
		String log = "";
		String tempInput = "";
		...
		for (int i = 0; i <= inputArray.size() - 1; i++) {
			FileInput.cropType tempArray = inputArray.get(i);
		...
		for (FileInput.cropType e : inputArray) {
			log = log + e.toString() + "\n";
		}
	...
	class SortbyCropStage implements Comparator<cropType> {
		public int compare(cropType a, cropType b) {
			return Integer.compare(a.cropStage, b.cropStage);
		}
	}
